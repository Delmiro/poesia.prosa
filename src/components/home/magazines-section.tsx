...
export function MagazinesSection({ magazines }: { magazines: Magazine[] }) {
  ...
  return (
    <section className="border-y border-border bg-card/30 px-4 py-14 md:px-6">
      <div className="mx-auto max-w-6xl">
        <h2 className="font-display text-3xl font-semibold">Revistas Digitais</h2>
        <p className="mt-2 text-muted">Edições em PDF para leitura online</p>
          ...
          {magazines.map((mag, i) => (
            ...
            >
              <div className="relative aspect-[3/4] bg-border/30">
                {mag.cover_image_url ? (
                  <Image src={mag.cover_image_url} alt={mag.title} fill className="object-cover" sizes="300px" />
                ) : (
              ...
              <div className="p-5">
                ...
                <Link href={`/revistas/${mag.slug}`} className="mt-4 block">
